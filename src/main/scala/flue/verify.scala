...
/** A bundle that extracts the verifying f-interpolator from FastTrack.
 */
class Verify(val c: Context) {

  import c.universe.{ Match => _, _ }
  import definitions._

  @inline protected final def truly(body: => Unit): Boolean = { body ; true }
  @inline protected final def falsely(body: => Unit): Boolean = { body ; false }

  protected def fail(msg: String) = c.abort(c.enclosingPosition, msg)
   ...
   */
  def f(parts: List[Tree], args: List[Tree], origApplyPos: c.universe.Position): Tree = {
